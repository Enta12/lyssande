...
type Props = {
	...
	playerIndex: number;
	onSelectedPc: (playerIndex: number, pcID: string) => void;
	quest: string;
	disable?: boolean;
	platform: Platform;
};
...
const PcSessionSelector = ({
	...
	playerName,
	selectedPc,
	onSelectedPc: handleSelectedPc,
	playerIndex,
	disable = false,
	platform,
}: Props) => {
	const [isOpen, setIsOpen] = useState(false);
	const handleClick = (pcIndex: string, e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
		handleSelectedPc(playerIndex, pcIndex);
		setIsOpen(false);
		e.stopPropagation();
	};
			...
			header={
				<div className="flex justify-between flex-1">
					...
					<div className="flex justify-end gap-28">
						{selectedPc && <SelectedPc level={selectedPcData.level} name={selectedPcData.name} />}
					</div>
				</div>
			}
		>
