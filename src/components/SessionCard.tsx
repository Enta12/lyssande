...
const SessionCard = ({ data, users }: Props) => {
	...
	return (
			...
			header={
				<div className="flex gap-2 items-center justify-between w-full">
					<div className="flex gap-2 items-center">
						<img src={platformIcon[data.platform].icon} alt={platformIcon[data.platform].alt} />
						<span>
                ...
                `}
					...
					{gm && <span className="min-w-max mr-4">Organisé par {gm.name.toLocaleUpperCase()}</span>}
				</div>
			}
			isOpen={isOpen}
			onOpen={setIsOpen}
		>
			<div className="relative">
				<hr />
				<p
					className={cn('text-white font-bubblegum my-4 whitespace-pre-wrap', {
				...
				<p className="font-lg p-5 text-white font-bubblegum">
					...
					{data.characters.map((characterData, index) => {
						const character = users[users.findIndex((user) => user.id === characterData.player)];
						return (
