	...
	return (
		<div className="flex flex-col gap-3 w-full">
			...
			<div className="flex gap-2">
				{session && (
					<img src={platformIcon[session.platform].icon} alt={platformIcon[session.platform].alt} />
				)}
				<p className="text-2xl p-5 text-swamp font-bubblegum">
					{gm && (
						<span className="mr-4 text-darkBrown">Organisé par {gm.name.toLocaleUpperCase()}</span>
					)}
					AVEC
					{session &&
