...
type Props = {
	...
	onChange: (protagonist: Protagonist, cou: boolean, ambidexterity: boolean) => void;
	onDelete: () => void;
};
...
const ProtagonistForm = ({
	...
	const onStatChange = (action: 'prd' | 'at' | 'cou' | 'secondAt', newValue: number) => {
		const protagonistTemp = protagonist;
	...
	return (
		<div className="bg-orange p-2 h-56 w-[147px] rounded-2xl">
			<div
				className="
            rounded-2xl
            bg-beige
            ...
            focus:outline-none"
				...
				<div className="flex justify-between items-center mx-1.5 ">
					...
					<div className="flex justify-between items-center">
						{protagonist.secondAt !== undefined && (
							<ProtagonistInput
								type="text"
								handleChange={(e) => onStatChange('secondAt', parseInt(e.target.value) || 0)}
								value={protagonist.secondAt}
							/>
						)}
						<ProtagonistInput
							type="text"
							handleChange={(e) => onStatChange('at', parseInt(e.target.value) || 0)}
				...
				<div className="flex justify-between items-center mx-1.5 ">
					...
					<ProtagonistInput
						type="number"
						handleChange={(e) => onStatChange('prd', parseInt(e.target.value) || 0)}
			...
			<div className="flex flex-col bg-orange py-1.5 gap-2">
                ...
                h-6
                border-4
                ${protagonist.secondAt ? 'bg-green-500' : 'bg-brown'}
                border-white
                rounded-lg`}
					/>
				</div>
